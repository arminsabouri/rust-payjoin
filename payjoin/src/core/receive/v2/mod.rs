...
impl Receiver<Initialized> {
    ...
    pub(crate) fn apply_unchecked_from_payload(
        ...
        if self.session_context.expiry < SystemTime::now() {
            // Session is expired, close the session
            return Err(InternalReplayError::SessionExpired(self.session_context.expiry));
        }
...
impl Receiver<MaybeInputsSeen> {
    ...
    pub fn check_no_inputs_seen_before(
        ...
        MaybeFatalTransition::success(
            ...
            Receiver { state: OutputsUnknown { original: self.original.clone() }, session_context: self.session_context.clone() },
        )
    }

    pub(crate) fn apply_outputs_unknown(self) -> ReceiveSession {
        let new_state = Receiver { state: OutputsUnknown { original: self.original.clone() }, session_context: self.session_context.clone() };
...
impl Receiver<OutputsUnknown> {
    ...
    pub fn identify_receiver_outputs(
        ...
        MaybeFatalTransition::success(
            ...
            Receiver { state: WantsOutputs { inner }, session_context: self.session_context.clone() },
        )
    }

    pub(crate) fn apply_wants_outputs(self, inner: common::WantsOutputs) -> ReceiveSession {
        let new_state = Receiver { state: WantsOutputs { inner }, session_context: self.session_context.clone() };
...
impl Receiver<WantsOutputs> {
    ...
    pub fn substitute_receiver_script(
        ...
        Ok(Receiver { state: WantsOutputs { inner }, session_context: self.session_context.clone() })
    }
    ...
    pub fn commit_outputs(self) -> NextStateTransition<SessionEvent, Receiver<WantsInputs>> {
        ...
        NextStateTransition::success(
            ...
            Receiver { state: WantsInputs { inner }, session_context: self.session_context.clone() },
        )
    }

    pub(crate) fn apply_wants_inputs(self, inner: common::WantsInputs) -> ReceiveSession {
        let new_state = Receiver { state: WantsInputs { inner }, session_context: self.session_context.clone() };
...
impl Receiver<WantsInputs> {
    ...
    pub fn contribute_inputs(
        ...
        Ok(Receiver { state: WantsInputs { inner }, session_context: self.session_context.clone() })
    }
    ...
    pub fn commit_inputs(self) -> NextStateTransition<SessionEvent, Receiver<WantsFeeRange>> {
        ...
        NextStateTransition::success(
            ...
            Receiver { state: WantsFeeRange { inner }, session_context: self.session_context.clone() },
        )
    }

    pub(crate) fn apply_wants_fee_range(self, inner: common::WantsFeeRange) -> ReceiveSession {
        let new_state = Receiver { state: WantsFeeRange { inner }, session_context: self.session_context.clone() };
...
impl Receiver<WantsFeeRange> {
    ...
    pub fn apply_fee_range(
        ...
        MaybeFatalTransition::success(
            ...
            Receiver { state: ProvisionalProposal { psbt_context }, session_context: self.session_context.clone() },
        )
    }

    pub(crate) fn apply_provisional_proposal(self, psbt_context: PsbtContext) -> ReceiveSession {
        let new_state = Receiver { state: ProvisionalProposal { psbt_context }, session_context: self.session_context.clone() };
...
impl Receiver<ProvisionalProposal> {
    ...
    ) -> MaybeTransientTransition<SessionEvent, Receiver<PayjoinProposal>, ImplementationError>
    {
        ...
        MaybeTransientTransition::success(
            ...
            Receiver { state: payjoin_proposal, session_context: self.session_context.clone() },
        )
    }

    pub(crate) fn apply_payjoin_proposal(self, psbt: Psbt) -> ReceiveSession {
        let new_state = Receiver { state: PayjoinProposal { psbt }, session_context: self.session_context.clone() };
...
pub mod test {
    ...
    pub(crate) fn mock_err() -> (String, JsonReply) {
        ...
        let receiver = Receiver { state: unchecked_proposal_v2_from_test_vector(), session_context: SHARED_CONTEXT.clone() };
        let server_error = || {
            receiver
                .clone()
    ...
    fn test_v2_mutable_receiver_state_closures() {
        ...
        let receiver = v2::Receiver { state: maybe_inputs_owned, session_context: SHARED_CONTEXT.clone() };
    ...
    fn test_unchecked_proposal_transient_error() -> Result<(), BoxError> {
        ...
        let receiver = v2::Receiver { state: unchecked_proposal, session_context: SHARED_CONTEXT.clone() };
    ...
    #[test]
    fn test_v2_pj_uri() {
        let uri = Receiver { state: Initialized {}, session_context: SHARED_CONTEXT.clone() }.pj_uri();
    ...
    fn test_unchecked_from_payload_disables_output_substitution_for_v1() {
        ...
        let mut receiver = Receiver { state: Initialized {}, session_context: SHARED_CONTEXT.clone() };
